...
st.title("🧾 旅費拆帳小幫手")
# 新增了一個「🔧系統測試」的分頁
tab1, tab2, tab3, tab4 = st.tabs(["📸 收據辨識", "✍️ 手動輸入", "📊 結算匯出", "🔧 系統測試"])
        ...
        if st.session_state.ai_processed and st.session_state.current_receipt_data:
            ...
            with st.form("receipt_form"):
                ...
                for i, item in enumerate(data.get("items", [])):
                    ...
                    with c2:
                        consumer = st.selectbox("誰的消費？", options=CONSUMERS, key=f"r_item_{i}", label_visibility="collapsed")
                    
                    selections.append({
                        "日期": datetime.now().strftime("%Y-%m-%d"), "品項": item['translated_name'],
                        "日幣金額": item['price'], "付款方式": payment_method,
                        "代墊者": payer, "消費者": consumer
                    })
                    st.write("---")
...
# === 分頁 4：系統測試 (除錯專用) ===
with tab4:
    st.subheader("🔧 API 狀態偵測")
    if st.button("點我測試 API 能抓到哪些模型", type="primary"):
        try:
            # 嘗試抓取 API Key 可用的模型列表
            available_models = [m.name for m in genai.list_models() if 'generateContent' in m.supported_generation_methods]
            st.success("✅ API 連線成功！你的金鑰支援以下模型：")
            st.write(available_models)
        except Exception as e:
            st.error(f"❌ API 連線失敗或權限錯誤，請檢查 Secret Key。錯誤代碼：{e}")import streamlit as st
import pandas as pd
from datetime import datetime
import json
import google.generativeai as genai
